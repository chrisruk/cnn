...
with tf.Session(config=config) as sess:
    ...
    if CNN1:
        network = conv_2d(network, 64,[1,3], activation='relu',name="conv1",regularizer="L1")
        network = conv_2d(network, 16,[2,3], activation='relu',name="conv2",regularizer="L1")
    ...
    model.fit(X, Y, n_epoch=500, validation_set=0.0,shuffle=True,show_metric=True, batch_size=1024, run_id='radio_cnn')
